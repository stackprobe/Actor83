...
void Gnd_INIT(void)
{
	...
	{
		Gnd.PlayData.TakaraOpenTable[x][y] = new bitList();
	}
}
void Gnd_FNLZ(void)
{
	...
	for(int y = 0; y < MAP_H; y++)
	{
		delete Gnd.PlayData.TakaraOpenTable[x][y];
	}
}
...
#define SAVEDATA_SIGNATURE "Actor83_SaveData"
...
void ImportSaveData(void)
{
	...
	Gnd.BestTimeFrame = SD_ReadInt(0, PLAYTIMEFRAME_MAX);

	Gnd.PlayData.SavedFlag = SD_ReadBoolean();
	Gnd.PlayData.MapX = SD_ReadInt(0, MAP_W - 1);
	Gnd.PlayData.MapY = SD_ReadInt(0, MAP_H - 1);
	Gnd.PlayData.EnterX = SD_ReadInt(0, IMAX);
	Gnd.PlayData.EnterY = SD_ReadInt(0, IMAX);
	Gnd.PlayData.PlayTimeFrame = SD_ReadInt(0, PLAYTIMEFRAME_MAX);
	Gnd.PlayData.ClearTimeFrame = SD_ReadInt(0, PLAYTIMEFRAME_MAX);

	for(int x = 0; x < MAP_W; x++)
	for(int y = 0; y < MAP_H; y++)
	{
		SD_ReadBitList(Gnd.PlayData.TakaraOpenTable[x][y]);
	}
	Gnd.FieldDaylightDisabled = SD_ReadBoolean();
	Gnd.ServerConnectDisabled = SD_ReadBoolean();
...
void ExportSaveData(void)
{
	...
	SD_WriteInt(Gnd.BestTimeFrame);

	SD_WriteInt(Gnd.PlayData.SavedFlag);
	SD_WriteInt(Gnd.PlayData.MapX);
	SD_WriteInt(Gnd.PlayData.MapY);
	SD_WriteInt(Gnd.PlayData.EnterX);
	SD_WriteInt(Gnd.PlayData.EnterY);
	SD_WriteInt(Gnd.PlayData.PlayTimeFrame);
	SD_WriteInt(Gnd.PlayData.ClearTimeFrame);

	for(int x = 0; x < MAP_W; x++)
	for(int y = 0; y < MAP_H; y++)
	{
		SD_WriteBitList(Gnd.PlayData.TakaraOpenTable[x][y]);
	}
	SD_WriteInt(Gnd.FieldDaylightDisabled);
	SD_WriteInt(Gnd.ServerConnectDisabled);
