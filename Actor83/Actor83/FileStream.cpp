...
#if LOG_ENABLED == 0
	// ? リリース版で tmp, temp を参照している。-> 何かおかしい。
	errorCase(!_strnicmp(file, "C:\\tmp\\", 7));
	errorCase(!_strnicmp(file, "C:\\temp\\", 8));
#endif
...
char *readCfgLine(autoList<uchar> *fileData, int &rIndex)
{
	for(; ; )
	{
		char *line = anReadLine(fileData, rIndex);
...
void readBlock(autoList<uchar> *fileData, int &rIndex, uchar *buff, int size)
{
	for(int c = 0; c < size; c++)
	{
		buff[c] = fileData->GetElement(rIndex++);
	}
}
void writeBlock(autoList<uchar> *fileData, uchar *data, int size)
{
	for(int c = 0; c < size; c++)
	{
		fileData->AddElement(data[c]);
	}
}
