...
static void ConfigInit(void)
{
	...
	Config.ServerHost = readCfgLine(fileData, rIndex);
	Config.ServerPort = atoi_x(readCfgLine(fileData, rIndex));

	delete fileData;

	errorCase(m_isEmpty(Config.ServerHost));
	errorCase(!m_isRange(Config.ServerPort, 1, 65535));
}

#define RECVSIZE_MAX 1000000
#define CONNECT_PERIOD_MILLIS 5000
...
{
	ConfigInit();
	return Config.ServerPort;//58946;
}
...
static int DoSendData(int sock, void *data, int dataSize)
{
	...
	uncritical();
	{
		retval = send(sock, (const char *)data, dataSize, 0);
	}
	critical();
...
static int DoRecvData(int sock, void *data, int dataSize)
{
	...
	uncritical();
	{
		retval = recv(sock, (char *)data, dataSize, 0);
	}
	critical();
...
static __int64 NextConnectTime;

// ---- Comm_End ----

static int Comm_ErrorCount;

static void Comm_End(void)
{
	NextConnectTime = FrameStartTime + CONNECT_PERIOD_MILLIS;
}
...
//			ErrorMessage = "";
		...
		if(FrameStartTime < NextConnectTime) // 通信間隔調整
		{
...
{
	__int64 maxTime = FrameStartTime + millisMax;
	m_minim(NextConnectTime, maxTime);
}
int GetSockConnectWaitMillis(void)
{
	__int64 waitTime = NextConnectTime - FrameStartTime;
	m_range(waitTime, -1, IMAX);
	return (int)waitTime;
}
